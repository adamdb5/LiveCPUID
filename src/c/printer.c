...
void print_fn0000_0001(CPUID *cpuid, unsigned int *current_line) {
  ...
  write_string("Largest Standard Function Number:", 40, *current_line, FG_WHITE);
...
void print_fn0000_0005(CPUID *cpuid, unsigned int *current_line) {
  ...
  char num_string[9];
  str_len_a = int_to_string(cpuid->monitor_mwait_features.monitor_line_size_min, num_string, 10);
  (*current_line)++;
  ...
  str_len_b = int_to_string(cpuid->monitor_mwait_features.monitor_line_size_max, num_string, 10);
